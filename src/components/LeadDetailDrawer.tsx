...
const LeadDetailDrawer = ({ lead, open, onClose }: Props) => {
  ...
  const handleStatusChange = async (status: string) => {
    try {
      ...
      toast.success(
        `Status updated to ${PIPELINE_STAGES.find((s) => s.key === status)?.label}`,
      );
    } catch (err: any) {
  ...
  const formatAction = (action: string, metadata: any) => {
    switch (action) {
      ...
      case "visit_scheduled":
        return `Visit scheduled for ${metadata.scheduled_at ? format(new Date(metadata.scheduled_at), "MMM d, h:mm a") : "TBD"}`;
      case "visit_outcome":
          ...
          <div className="grid grid-cols-2 gap-3 mt-4">
            ...
            <div className="flex items-center gap-2 text-xs text-muted-foreground">
              ...
              {lead.first_response_time_min != null
                ? `${lead.first_response_time_min}m response`
                : "No response yet"}
            </div>
            <div className="flex items-center gap-2 text-xs text-muted-foreground">
          ...
          <TabsContent value="timeline" className="mt-4 space-y-2">
                ...
                {lead.first_response_time_min != null && (
                  <div className="flex items-start gap-3 p-3 rounded-lg bg-secondary/50">
                    ...
                    <div>
                      ...
                      <p className="text-[10px]">
                        {lead.first_response_time_min} minutes after creation
                      </p>
                    </div>
                  </div>
                )}
            ...
            {lead.notes && (
              <div className="p-3 rounded-lg bg-accent/50 border border-accent">
                <p className="text-[10px] font-medium text-accent-foreground mb-1">
                  Notes
                </p>
                ...
                >
                  <div className="flex items-center justify-between mb-1">
                    <span className="font-medium text-foreground capitalize">
                      {c.direction === "inbound" ? lead.name : "Agent"}
                    </span>
                ...
                >
                  <div className="flex items-center justify-between">
                    <span className="font-medium text-foreground flex items-center gap-1">
                      <Bell size={10} />{" "}
                      {format(new Date(f.reminder_date), "MMM d, h:mm a")}
                    </span>
                    <Badge
                  ...
                  {f.note && (
                    <p className="text-muted-foreground mt-1">{f.note}</p>
                  )}
                </div>
              ))}
              {followUps?.length === 0 && (
